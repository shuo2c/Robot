...
**三重保险（互不依赖、互为兜底）：**
- **定时提交**：`cron`（或系统定时任务）每 N 分钟跑一次 `bash sync.sh cron`，兜底断电/崩溃——你来不及手动 sync，定时任务替你存。
- **会话停止 hook**：`.claude/settings.json` 里的 `Stop` hook 会在 Claude Code 会话停止时自动触发 `sync.sh`。这是"睡去前 sync"的自动兜底——即使你忘了，hook 替你执行。
